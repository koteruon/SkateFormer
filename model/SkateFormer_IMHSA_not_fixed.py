...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, in_channels, rel_type, num_heads=32, partition_size=(1, 1), attn_drop=0.0, rel=True):
        ...
        self.seq_len = partition_size[0] * partition_size[1]
        ...
        self.f_Qk2 = nn.Linear(num_heads, num_heads)
        self.f_qK1 = nn.Linear(num_heads, num_heads)
        self.f_qK2 = nn.Linear(num_heads, num_heads)
    ...
    def interaction(self, attn, layer):
        attn = attn.permute(1, 2, 0)  # (h, N, L) -> (N, L, h)
        attn = layer(attn)
        return attn.permute(2, 0, 1)  # (N, L, h) -> (h, N, L)
    ...
    def forward(self, input):
        ...
        # qK attention
        qK = torch.matmul(q, K.transpose(-2, -1)) * self.scale  # (h, L, N)
        qK = self.interaction(qK, self.f_qK1)
        qK = self.softmax(qK)
        qK = self.interaction(qK, self.f_qK2)
        ...
        if self.rel:
            bias = self._get_relative_positional_bias()  # (1, h, N, N)
            Qk = Qk + bias.squeeze(0)[:, : Qk.size(1), : Qk.size(2)]

        # Final attention
        out = torch.matmul(Qk, torch.matmul(qK, V))  # (h, N, d)
        out = out.permute(1, 0, 2).reshape(B, N, -1)
        out = self.f_cat(out)
        return out
