...
class SkateFormerBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        M = self.M
        f_conv = f_conv.view(B, C // 2, T, M, V // M).permute(0, 3, 1, 2, 4).contiguous().view(B * M, C // 2, T, V // M)
        f_conv_output = self.sgcn(f_conv)
        f_conv_output = f_conv_output.view(B, M, C // 2, T, V // M).permute(0, 2, 3, 1, 4).contiguous().view(B, C // 2, T, V)
